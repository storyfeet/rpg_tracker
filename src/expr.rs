...
impl Expr {
    ...
    pub fn from_tokens(it: &mut Tokenizer) -> Result<Expr, ParseError> {
        ...
        while let Some(t) = it.next(){
            match t {
                ...
                Token::Dot=>{
                    match it.next(){
                        Some(Token::Ident(s))=> {
                            ...
                            .push_ident(&s).map_err(|p|p.set_line(it.line_no))?;
                        }
                        None|Some(_)=>return Err(ParseError::new("Expected idend after dot",it.line_no)),
                ...
                _=> return Err(ParseError::new("Unexptected token in expression",it.line_no)),
            }
        }

        let p = Self::split_op(parts,Token::Mul,|a,b|Expr::Mul(Box::new(a),Box::new(b)))?;
    ...
    pub fn split_op<IT,F>(i:IT,t:Token,f:F)->Result<Vec<Expr>,ParseError>
        where IT:IntoIterator<Item=Expr>+std::fmt::Debug,
              F:Fn(Expr,Expr)->Expr,
    {
        println!("{:?}",i);
        // mul and div
        ...
        while let Some( p) = pit.next(){ 
            if p == Expr::Op(t.clone()){
                    a = Some(f(
                            ...
                            pit.next().ok_or(ParseError::new("Nothing after the *",0))?));
            } else{
                if let Some(prev) = a {
                    res.push(prev)
                }
